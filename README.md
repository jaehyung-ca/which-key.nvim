...
```lua
{
  "jaehyung-ca/shortcuts.nvim",
  event = "VeryLazy",          -- load before you first reach for a mapping
  opts = {                     -- passed straight to require("shortcuts").setup()
...
```sh
git clone https://github.com/jaehyung-ca/shortcuts.nvim \
  "${XDG_CONFIG_HOME:-$HOME/.config}/nvim/pack/plugins/start/shortcuts"
```
...
Generate the help tags once so `:help shortcuts` works:
...
If the code already lives on disk (e.g. `~/src/shortcuts`), just add it to your `runtimepath` and call `setup()`:

```lua
vim.opt.runtimepath:append("~/src/shortcuts")
require("shortcuts").setup({ delay = 200 })
```
...
| Command | Description |
|---|---|
| `:Shortcuts [prefix]` | Open the popup for a prefix (default leader) |
| `:ShortcutsSearch` | Live fuzzy search over all bindings |
| `:ShortcutsWhat` | Show recent raw keystrokes (the panic buffer) |

## Why a separate "panic buffer"?

The action a key triggers and the *raw key you pressed* are different problems. Reviewing registered actions tells you about shortcuts you set up on purpose; it can't explain a stray keypress, because that key was never registered. So shortcuts records every keystroke verbatim via `vim.on_key` (no interpretation, just a bounded ring) and warns the instant `q` starts a macro recording — the most common "it suddenly went weird" cause.

## Configuration

Defaults (see `:help shortcuts-config`):
...
require("shortcuts").setup({
  ...
  search = {                 -- :ShortcutsSearch float size
...
Out of the box shortcuts only knows the bindings *you* register. The `presets`
...
```vim
:Shortcuts g        " menu of built-in g-shortcuts
:Shortcuts z        " menu of built-in z-shortcuts
:ShortcutsSearch    " fuzzy-search every known binding, built-ins included
```
...
live trigger on them is deferred (`:help shortcuts-limitations`). Use the
...
Highlight groups (all overridable, see `:help shortcuts-highlights`):
`ShortcutsKey`, `ShortcutsDesc`, `ShortcutsGroup`, `ShortcutsSeparator`,
`ShortcutsSearchMatch`, `ShortcutsSearchSel`, `ShortcutsSearchPrompt`.

## Status

Implemented: registry, `register`/`annotate`, leader popup + triggers, fuzzy search, keylog + macro guard, `:help` docs.

Deferred (see `:help shortcuts-limitations`): buffer-local map tracking; safe replay for non-leader (`g`/`z`) triggers; visual/insert-mode execution from search.
